...
from filtersA import butter_fir_filterA
from eigA import gevd

def csp_one_oneA(cov_matrix,NO_csp,NO_classes):
	'''
...
def generate_projectionA(data,class_vec,NO_csp,filter_bank,time_windows,NO_classes=4):
	...
	for t_wind in range(0,NO_time_windows):
		...
		for subband in range(0,NO_bands):
			...
			for trial in range(0,NO_trials):
				...
				data_filter = butter_fir_filterA(data[trial,:,t_start:t_end], filter_bank[subband])
			...
			w[t_wind,subband,:,:] = csp_one_oneA(cov_avg,NO_csp,NO_classes)
	return w


def generate_eyeA(data,class_vec,filter_bank,time_windows):
...
def extract_featureA(data,w,filter_bank,time_windows):
	...
	for trial in range(0,NO_trials):
		...
		for t_wind in range(0,NO_time_windows):
			...
			for subband in range(0,NO_bands):
				...
				#frequency filtering
				cur_data_f_s = butter_fir_filterA(cur_data_s,filter_bank[subband])
