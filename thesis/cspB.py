...
from filtersB import butter_fir_filterB
from eigB import gevd

def csp_one_oneB(cov_matrix,NO_csp,NO_classes):
	'''
...
def generate_projectionB(data,class_vec,NO_csp,filter_bank,time_windows,NO_classes=2):
	...
	'''print("Time Window ",time_windows)
	print("No of Bands",NO_bands)
	print("No of time windows",NO_time_windows)
	...
	for t_wind in range(0,NO_time_windows):
		...
		for subband in range(0,NO_bands):
			...
			for trial in range(0,NO_trials):
				...
				data_filter = butter_fir_filterB(data[trial,:,t_start:t_end], filter_bank[subband])
				#print("Type of data filter ",type(data_filter))
				#print("Is Null value available ",np.isnan(data_filter))
				cur_class_idx = int(class_vec[trial]-1)
				#print("Is Null value available ",np.isnan(cur_class_idx))
			...
			w[t_wind,subband,:,:] = csp_one_oneB(cov_avg,NO_csp,NO_classes)
	return w


def generate_eyeB(data,class_vec,filter_bank,time_windows):
...
def extract_featureB(data,w,filter_bank,time_windows):
	...
	for trial in range(0,NO_trials):
		...
		for t_wind in range(0,NO_time_windows):
			...
			for subband in range(0,NO_bands):
				...
				#frequency filtering
				cur_data_f_s = butter_fir_filterB(cur_data_s,filter_bank[subband])
